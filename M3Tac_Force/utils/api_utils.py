...
def depth2rgb(depth_img, min_depth=0.0, max_depth=1.5, color_mode=cv2.COLORMAP_JET, reverse_scale=False,
    ...
    if dynamic_scaling:
        depth_img_scaled = _normalize_depth_img(depth_img, dtype=np.uint8,
                                                min_depth=max(depth_img[depth_img > 0].min(), min_depth),
...
def dict_to_table(Dict, logger, table_name):
    key = list(Dict.keys())
    value = list(Dict.values())
    value = ['%.4f' % x for x in value]
    showdata = []
    showdata.append(value)
    logger.info('{}\n{}'.format(table_name, tabulate(showdata, headers=key, tablefmt='fancy_grid', disable_numparse=True)))
